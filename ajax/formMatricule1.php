<?php
    ...
    $reg = $query->fetch_row();
    $checkInadimplente = $reg[25];

    if ($checkInadimplente == 1) {
        
        $query2 = $db->query("SELECT * FROM tbl_unidades WHERE inadimplente='0' AND fila='0' AND ativo='0' LIMIT 0, 1");
        $count2 = $query2->fetch_row();

        if ($count2 == 0) {
            $query3 = $db->query("UPDATE tbl_unidades SET fila='0'");
            
            $query4 = $db->query("SELECT * FROM tbl_unidades WHERE inadimplente='0' AND fila='0' AND ativo='0' LIMIT 0, 1");
            $reg4 = $query4->fetch_row();
            $unidade = $reg4[1];
            $checkEmailComercial = $reg4[20];
            $checkEmailCopia1 = $reg4[21];
            $checkEmailCopia2 = $reg4[22];

            $emailEnvio = $checkEmailComercial;

            $query5 = $db->query("UPDATE tbl_unidades SET fila='1' WHERE unidade='$unidade'");
        } else {
            $reg2 = $query2->fetch_row();
            $unidade = $reg2[1];
            $checkEmailComercial = $reg2[20];
            $checkEmailCopia1 = $reg2[21];
            $checkEmailCopia2 = $reg2[22];
        ...
        $checkEmailComercial = $reg[20];
        $checkEmailCopia1 = $reg[21];
        $checkEmailCopia2 = $reg[22];

        $emailEnvio = $checkEmailComercial;
    }

    if (empty($checkEmailCopia1)) {
        $emailCopia = 'comercial@united18.com.br';
    } else {
        $emailCopia = $checkEmailCopia1;
    }

    if (empty($checkEmailCopia2)) {
        $emailCopia2 = 'comercial@united18.com.br';
    } else {
        $emailCopia2 = $checkEmailCopia2;
    }
