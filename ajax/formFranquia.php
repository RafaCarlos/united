<?php
    ...
    // $sql = mysql_query("INSERT INTO tbl_mensagem_franquias (nome, telefone, email, data_mensagem) 
    //     VALUES('{$nome}', '{$telefone}', '{$email}', now())") 
    //     or die( mysql_error() );

    $insert = $db->query("INSERT INTO tbl_mensagem_franquias (nome, telefone, email, data_mensagem) VALUES('{$nome}', '{$telefone}', '{$email}', now())");
    ...
    if ($db->affected_rows > 0){
        ...
        $mensagem = '<html><body>';
        $mensagem .= '<h1 style="font-size:15px;">' . utf8_decode('Seja um Franqueado - Contato') . '</h1>';
        ...
        $mail->Subject = utf8_decode('Seja um Franqueado - Contato');
