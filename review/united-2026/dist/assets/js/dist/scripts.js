...
;
/* Video playback is maintained in /media-runtime.js. */
...
var structure = {
    ...
    clickMenu: function(e) {
        structure.navigateAnchor(e, 100);
	},

    clickMenuMobi: function(e) {
        structure.navigateAnchor(e, 106);
	},

    navigateAnchor: function(e, offset) {
        var link = e.currentTarget;
        if (!link || e.ctrlKey || e.metaKey || e.shiftKey || e.altKey ||
            (e.which && e.which !== 1) || e.defaultPrevented ||
        ...
        var normalize = function(path) {
            return path.replace(/\/index\.php$/, '/').replace(/\.php$/, '').replace(/\/+$/, '') || '/';
        };
        if (url.origin !== window.location.origin ||
            normalize(url.pathname) !== normalize(window.location.pathname) ||
            (url.search && url.search !== window.location.search) || !id) {
            return;
        }
        ...
        $('html, body').stop(true).animate({scrollTop: Math.max(0, $(target).offset().top - offset)}, reducedMotion ? 0 : 600);
        structure.closeMenu();
    ...
    loadFAQ: function() {
        ...
        // Enhance the server-rendered answers; no additional request is needed.
        faq.addClass('is-enhanced');
        faq.find('.question').removeClass('open');
        faq.find('.faq-toggle').attr('aria-expanded', 'false').on('click', structure.openItem);
        faq.find('.question .text').prop('hidden', true);
        faq.find('.top form').on('submit', function(e) {
            e.preventDefault();
            structure.filterFAQ();
        });
        faq.find('#busca').on('input', structure.filterFAQ);
        faq.find('aside a[data-faq-id]').on('click', function(e) {
            e.preventDefault();
            ...
            faq.find('#busca').val('');
            faq.find('.question').prop('hidden', true).removeClass('open');
            faq.find('.faq-toggle').attr('aria-expanded', 'false');
            faq.find('.question .text').prop('hidden', true);
            faq.find('.faq-empty').prop('hidden', true);
            $(selected).prop('hidden', false).addClass('open');
            $(selected).find('.text').prop('hidden', false);
            $(selected).find('.faq-toggle').attr('aria-expanded', 'true').trigger('focus');
        });

        structure.filterFAQ();
        if (/^#faq-[a-z0-9-]+$/.test(window.location.hash)) {
            var linked = document.getElementById(window.location.hash.substring(1));
            if (linked && faq[0].contains(linked)) {
                $(linked).prop('hidden', false).addClass('open');
                $(linked).find('.text').prop('hidden', false);
                $(linked).find('.faq-toggle').attr('aria-expanded', 'true');
                faq.find('.faq-empty').prop('hidden', true);
            }
        }
    },
    ...
    openItem: function(e) {
        var _this = $(e.currentTarget);
        var faq = _this.closest('.faq');
        var opening = _this.attr('aria-expanded') !== 'true';
        faq.find('.question').removeClass('open');
        faq.find('.faq-toggle').attr('aria-expanded', 'false');
        faq.find('.question .text').prop('hidden', true);
        if (opening) {
            ...
            _this.next('.text').prop('hidden', false);
        }
    },
    ...
    filterFAQ: function() {
        var faq = $('.faq');
        var search = $.trim(faq.find('#busca').val() || '').toLowerCase();
        var count = 0;
        faq.find('.question').each(function() {
            var question = $(this);
            var content = question.find('.faq-toggle').text() + ' ' + question.find('.text').text();
            var matches = content.toLowerCase().indexOf(search) !== -1;
            question.prop('hidden', !matches);
            if (matches) {
                count++;
            }
        });
        faq.find('.faq-empty').prop('hidden', count > 0);
    },

    openForm: function() {
        $('.box-form .success').prop('hidden', true).hide();
        $('.box-form').fadeIn();
    },
